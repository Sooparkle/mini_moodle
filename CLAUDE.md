...
- 단일 쿼리: `import { sql } from '@/lib/db'` → tagged template (`sql\`SELECT ...\``)
- 트랜잭션: `import { withTransaction } from '@/lib/db'` → `tx.query('SQL', [params])` (node-postgres API)
- 패키지: `@neondatabase/serverless` (neon HTTP + Pool WebSocket)
- `src/lib/db.ts`에서 `neon(POSTGRES_URL, { fullResults: true })`로 sql을 export — `{ rows }` 디스트럭처링 호환
- Postgres 백엔드: 직접 가입한 Neon 계정 소유 (Vercel 마켓플레이스 통합 미사용)
