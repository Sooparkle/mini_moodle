...
## Appendix A — Phase 9: 강좌 페이지 UX 비교 (Canvas + Mode-less)

> 추가: 2026-05-17 (Session 12~13 산출물)
> 본 부록은 Session 11에서 추가된 Phase 9(Course UX 비교)의 회고 기록이다.

### A.1 배경

Moodle 5.x opensource 강좌 페이지를 분석한 결과 5대 UX 약점이 식별되었다.

1. **편집/뷰 모드 전환 비용** — 교수가 학생 시점을 확인하려면 "편집 끄기 → 학생 페이지 → 편집 켜기" 3단계
2. **활동 진입 후 복귀 비용** — 활동 description 한 줄 보려고 페이지 이탈
3. **정보 위계 불분명** — 섹션·활동·블록 정보가 평탄하게 나열
4. **진척도 분산** — 학생이 자기 위치 파악 어려움 (블록 켜야 보임)
5. **잠금 사유 불친절** — Restrict Access 발동 시 학생에게 사유가 명확하지 않음 (5.2에서 개선됨)

Phase 9는 이 중 **(1) 편집/뷰 모드 전환 비용**과 **(2) 활동 진입 후 복귀 비용**을 정면 공격하는 두 변형을 만들어 기본 뷰와 비교하는 것이 목표.

### A.2 라우팅 구조

| 경로 | 변형 | 핵심 차별점 |
|------|------|------------|
| `/courses/[id]` | **기본** | 명시적 [편집] 버튼 토글 패턴, 학생 뷰는 별도 페이지 |
| `/courses/[id]/canvas` | **A: Single-canvas** | 3-pane(목차+본문+우측 정보 패널) + 활동 슬라이드오버 |
| `/courses/[id]/modeless` | **B: Mode-less** | hover-pen 인라인 편집 + 학생 미리보기 토글 + 5초 Undo 토스트 |

페이지 상단 ViewToggle로 3-way 전환 가능. 동일 코스를 다른 UX로 비교하기 위한 명시적 URL 분기.

### A.3 변형별 설계 의도

#### A (Canvas) — 활동 진입 후 복귀 비용 0
- **CourseIndex** (좌): IntersectionObserver scrollspy로 현재 섹션 highlight
- **SectionCanvas** (중): 활동 카드 그리드
- **RightRail** (우): 학생 진척도% + 다가오는 마감 + 최근 성적, 교수 통계
- **ActivityPeek**: 활동 카드 클릭 → 우측 슬라이드오버 (`?peek=N`). 페이지 이탈 없음
- 깊은 작업(퀴즈 응시, 채점)은 그대로 풀페이지 — Moodle의 "한 줄 보러 이탈" 패턴을 슬라이드오버로 흡수

#### B (Mode-less) — 편집/뷰 모드 전환 비용 0
- **InlineEditableText**: 텍스트 호버 시 펜 아이콘 노출, 1-click에 즉시 textarea 전환
- **hover-only 컨트롤**: 위↑↓× 버튼은 카드 호버 때만 opacity 1 — 학생 시점과 시각적으로 거의 동일
- **StudentPreviewToggle**: `?as=student` URL로 즉시 학생 미리보기, 다크 배너에 "편집으로 돌아가기"
- **5초 Undo 토스트**: 삭제 즉시 실행 X, 5초 카운트다운 후 서버 액션. 페이지 이탈 시 미실행 = "암묵 취소"

### A.4 차별점 요약

| 메트릭 | 기본 | A (Canvas) | B (Mode-less) |
|---|---|---|---|
| 섹션 제목 수정 클릭 수 | 3 (편집→폼→저장) | 3 (편집→폼→저장) | **2 (제목→Enter)** |
| 활동 description 확인 클릭 수 | 2 (링크→뒤로) | **1 (peek 슬라이드오버)** | 1 (인라인) |
| 학생 시점 확인 비용 | 별도 로그인 또는 비공개 모드 | 별도 로그인 | **`?as=student` 토글 1-click** |
| 삭제 실수 회복 | 불가 (confirm만) | 불가 | **5초 Undo** |
| 현재 위치(섹션) 인지 | 스크롤로 추정 | **scrollspy highlight** | 스크롤로 추정 |
| 진척도/마감 가시성 | 별도 페이지 | **상시 우측 패널** | 없음 |
| 컨텍스트 전환 횟수 (편집↔학생 보기) | 2회 (편집 끄기 + URL) | 2회 | **0회 (단일 토글)** |

### A.5 구현 비용

| 변형 | 신규 파일 | 신규 코드 라인 | 디자인 토큰 추가 | DB 변경 |
|------|----------|--------------|---------------|---------|
| A (Canvas) | 6개 (page + Index + Canvas + RightRail + Peek + css) | ~1,584 | 없음 | 추가 쿼리 1개 (최근 성적) |
| B (Mode-less) | 7개 (page + Editor + InlineEdit + Toast + ReadOnly + PreviewToggle + css) | ~1,300 | 없음 | 없음 |

두 변형 모두 기존 서버 액션(createSection / updateSection / deleteSection 등)을 **그대로 재사용**. 새 액션·새 테이블 없음 → 인프라 변경 0, 순수 UX 레이어 차별화.

### A.6 한계

- **A**: 슬라이드오버 안에서 깊은 작업(퀴즈 응시)은 결국 풀페이지 — 슬라이드오버는 "한 줄 description" 시나리오에서만 유효
- **B**: 5초 Undo는 페이지 이탈 시 미실행 → 의도된 "암묵 취소"지만 사용자가 의도와 다른 결과로 혼동 가능. 학생 데이터 FK 연쇄 삭제 시점 추적 안 됨
- **공통**: 측정 메트릭(실제 클릭 수, 작업 완료 시간)은 사용자 테스트 미실시 — 위 표는 설계 의도 기반 추정치
- 모바일 터치 환경에서 hover-pen은 항상 노출로 fallback(`@media (hover: none)`) — 데스크탑보다 시각적 노이즈 증가

### A.7 다음 단계 (선택)

1. **사용자 테스트**: 교수 3명에게 동일 시나리오("새 섹션 추가 → 제목 수정 → 활동 1개 추가")를 3 변형으로 수행시켜 실제 클릭/시간 측정
2. **A+B 통합**: Canvas 3-pane 안에서 Mode-less 인라인 편집 활성화 → 두 차별점 동시 충족
3. **Mode-less + Drag&Drop**: 화살표 정렬을 dnd로 교체 → reorder 클릭 수 추가 절감

### A.8 Phase 9 진척

| 단계 | 상태 | 산출물 |
|---|---|---|
| 9a A 변형 | DONE | `/courses/[id]/canvas/` 6 파일, tsc clean |
| 9b B 변형 | DONE | `/courses/[id]/modeless/` 7 파일, tsc clean |
| 9c 회고 | DONE | 본 부록 (FINAL_REPORT.md Appendix A) |
